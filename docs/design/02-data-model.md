...
**为什么 `authors` 是数组而不是原文案的 `author` 字符串**：外部元数据的作者天然是列表（多作者是常态）；合成一个字符串会丢掉结构，之后想按作者精确筛选、或做「同一作者的其他书」，都得反过来拆字符串。UI 展示时 `authors.join(' / ')` 即可，成本为零。
...
| I11 | 每个 `Book.authors` / `Book.tags` 是**字符串数组** | 收敛成字符串数组（字符串→单元素数组；剔除非字符串项）并记警告（B6） |
