...
### 11.19 搜索页「筛选」折叠（B11，2026-09-20）

**问题**（手机版实测，390px）：搜索页四个筛选控件加两行说明要吃掉大半屏，**第一条结果要滚到 ~1150px 才出现**——而搜索最常用的动作是打几个字。桌面端看不出来（1280px 宽横着排得下），是**修好截图脚本之后看手机版才发现**的。

**做法**：关键词留在外面；**位置 / 标签 / 副本状态收进 `Collapsible`**（复用 B7 那个组件，只加了一个 `countLabel`，让它能说「N 项生效」而不是「已填 N 项」）。

**关键一条：收起时必须说清「还有几项开着」。** 徽章显示 `筛选 · N 项生效`（N = 位置/标签/状态里非默认的个数；关键词不算——它一直看得见）。没有这个徽章就会出现"明明没搜到书，其实是被藏起来的筛选挡住了"这类最难查的问题。

**效果**（手机 390px 实测）：第一条结果从 ~1150px 提到 ~640px，首屏从"看不到结果"变成 2 张半卡片。

**验证方式**：徽章走的是 `count > 0` 分支，默认状态永远是 0，**截图看不出来**——所以用 CDP 驱动真实交互来验（`scratch/check-badge.mjs`，断言页面文本而非看图），两处都过：
- 搜索页：设一个位置筛选 → 收起 → `筛选 · 1 项生效`
- 新增页：填一个折叠区字段 → 收起 → `更多信息（可选）· 已填 1 项`

**踩过的坑**（记下来免得重踩）：校验脚本一开始用**新浏览器 profile**，里面书库是空的（位置选择器只剩「全部位置」）→ 筛选根本设不上 → 徽章"验出来不存在"。**驱动界面的脚本必须复用塞过样例数据的 profile**（`%TEMP%/edge-shots`）。

**改动**：`features/search/SearchPage.tsx`（294 行）、`app/Collapsible.tsx`（60 行）。不碰数据层、不改筛选语义、不动测试（477 不变）。
