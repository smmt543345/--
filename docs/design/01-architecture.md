...
| 元数据 | 用户自配的 AI 接口（05 §3） | 免费源 Open Library / Google Books **已否决**：2026-09-19 实测本机网络对两者均不可达，接进来是死代码，见 §7 决策 D11 |
| 桌面打包 | Tauri 2 | 产物体积小 |
...
### 5.5 阶段 D 受影响文件与尺寸标注（2026-09-19 定稿）

沿用 §5.1 的标注纪律：>300 行触发拆分审查、>500 行必须拆分；新增文件一律按 ≤300 行设计。

**新建（6 个；`scan.test.ts` 为交付时补入，见本节末实测）**：

| 文件 | 预计行数 | 内容 |
|---|---|---|
| `platform/scan.ts` | ~90 | html5-qrcode 动态 `import()`（不进首屏包）、摄像头启停与释放、失败翻译成人话（04 §11.12） |
| `features/books/isbn-scan.ts` | ~60 | 条码文本 → 规范化 ISBN；期刊条码 / 附加码 / 乱码的判别（可 `node --test`） |
| `features/books/isbn-scan.test.ts` | ~80 | 上者的用例 |
| `platform/scan.test.ts` | 交付时补 | `translateScanError` 的 7 个用例（开发期：它是纯函数却承载全部失败文案，没它「文案挂错档」无人拦） |
| `features/books/ScanDialog.tsx` | ~120 | 取景器弹层：开 / 关、错误态、识别成功后回填并停摄像头 |
| `features/books/IsbnField.tsx` | ~130 | ISBN 输入行：字段 + 扫码入口 + AI 补全入口 + 提示（05 §3.3 的第 2 入口） |

**改动**：`features/books/NewBookPage.tsx` 337 行（2026-09-19 实测；§5.4 的 338 是 2026-09-13 的数）→ 预算约 300 行、**实测 282 行**（见本节末实测表；**减量**：ISBN 字段与 AI 补全整块迁入 `IsbnField.tsx`，正落在 §5.1「跨 300 行、留待页面拆分」的欠账上，顺手还掉一部分）；`package.json` 36 行 → +1 → ~37 行；`package-lock.json` 仅依赖树变化。

**不动的既有件**：`platform/capabilities.ts`（`probeHost()` / `canScan()` 已存在，结构不变）；`app/ui.tsx` 的 `Modal` / `InlineError` 等基元直接复用（结构不变）。

**依赖方向**：`IsbnField → ScanDialog → platform/scan`；`IsbnField → features/books/isbn-scan → domain/isbn`；`IsbnField → platform/ai`（既有）。无新环。

**实现后实测（2026-09-19 交付）**：全部 ≤300 行，未新增超限文件；两处偏离预算均已说明。

| 文件 | 预计 | 实测 |
|---|---|---|
| `platform/scan.ts` | ~90 | 140（多出的 50 行是失败文案表 + 释放幂等，两者都是 04 §11.12 明确要求的） |
| `platform/scan.test.ts` | 交付时补 | 60 |
| `features/books/isbn-scan.ts` | ~60 | 49 |
| `features/books/isbn-scan.test.ts` | ~80 | 69 |
| `features/books/ScanDialog.tsx` | ~120 | 122 |
| `features/books/IsbnField.tsx` | ~130 | 161（多出的 31 行：AI 配置订阅、扫后查库分支、三个入口的禁用/提示态） |
| `features/books/NewBookPage.tsx` | 337 → ~300 | **282** |

NewBookPage 减到 282 行：页面级聚合的结构未变，§5.4 那笔拆分欠账继续留着（现已离 300 更远）。

### 5.6 B7 + D2 + D3 受影响文件与尺寸标注（2026-09-20 定稿）

沿用 §5.1 标注纪律：>300 行触发拆分审查、>500 行必须拆分；新增文件按 ≤300 行设计。三个批次（B7 新增页精简、D2 拍照识别、D3 连续录入）共用同一批文件，故合并标注。

**新建（5 个）**：

| 文件 | 预计行数 | 内容 |
|---|---|---|
| `app/Collapsible.tsx` | ~45 | 「更多信息」折叠区块（默认收起、展开态不持久化）。**独立成文件是硬约束逼的**：`ui.tsx` 已 496 行，塞进去破 500 行上限 |
| `features/books/LookupActions.tsx` | ~180 | 动作行三按钮（📷 扫码 / 🖼 拍照识别 / ✦ AI 补全）+ 隐藏解码容器 + 提示横幅 + 扫后链路（查库 → 已有则提示、无则 AI 补全）（04 §11.12/§11.13） |
| `features/books/MoreFields.tsx` | ~150 | 「更多信息」区九个字段：ISBN、出版日期、标签、品相、所有者、备注、封面图 URL、拍照存封面、副本数量（04 §11.13） |
| `features/books/continuous-scan.ts` | ~90 | 纯逻辑：跳过已有判定、下一本草稿重置、累计与收尾文案（可 `node --test`）（04 §11.14） |
| `features/books/continuous-scan.test.ts` | ~80 | 上者的用例 |

**删除（1 个）**：`features/books/IsbnField.tsx`（161 行）—— 动作行拆入 `LookupActions.tsx`、ISBN 文本框并入 `MoreFields.tsx`。删除优先于新增；D 阶段的交付记录（§5.5）保留为历史。

**改动（4 个）**：

| 文件 | 现 → 预计 | 改什么 |
|---|---|---|
| `platform/scan.ts` | 140 → ~185 | +`scanImageFile()`（动态 import、本地解码、用完 `clear()`）+ 两条照片失败文案（D2） |
| `platform/scan.test.ts` | 60 → ~78 | +照片失败文案用例 |
| `features/books/ScanDialog.tsx` | 122 → ~200 | +「连续录入」开关、建书循环、累计 UI（D3） |
| `features/books/NewBookPage.tsx` | 282 → ~260 | 默认层编排、折叠区挂载、保存后不跳转 + 横幅（B7/D3）；字段整块移出，**行数下降** |

**依赖方向**：`NewBookPage → LookupActions → platform/scan`；`NewBookPage → MoreFields → CoverPicker`；`LookupActions → ScanDialog → continuous-scan`；`IsbnField` 删除。无新环。

**测试账**：基线 444 → 预计 ~470（照片失败文案约 3 例 + 连续录入纯逻辑约 20 例）。

**实现后实测（2026-09-20 交付）**：全部 ≤300 行、无跨档；三处超预算均已说明。

| 文件 | 预计 | 实测 |
|---|---|---|
| `app/Collapsible.tsx` | ~45 | 53 |
| `features/books/LookupActions.tsx` | ~180 | 233（多出的 53 行：照片识别入口与隐藏容器、AI/照片两个 busy 态与统一 pending 门、连录收尾提示） |
| `features/books/MoreFields.tsx` | ~150 | 139 |
| `features/books/continuous-scan.ts` | ~90 | 142（多出的 52 行：`aiPatchForBook` 只补空字段的判空逻辑 + 各类跳过/失败文案） |
| `features/books/continuous-scan.test.ts` | ~80 | 197（用例覆盖比预算多：跳过分支 4 种 + 计数/回退/队列） |
| `platform/scan.ts` | 140 → ~185 | 213 |
| `platform/scan.test.ts` | 60 → ~78 | 92 |
| `features/books/ScanDialog.tsx` | 122 → ~200 | 268（多出的 68 行：连录开关、建书循环、AI 串行队列、重复帧守卫与累计 UI） |
| `features/books/NewBookPage.tsx` | 282 → ~260 | **280** |

**测试账实测**：444 → **469**（+25），与预算 ~470 相符。

### 5.7 B8 受影响文件与尺寸标注（2026-09-20 定稿）

沿用 §5.1 纪律。本批次三件事：③ 文档订正（01 §8 B2 行 + README 的滞后状态）、① xlsx 换 SheetJS 官方 CDN 版（`0.20.3`，URL 自带版本号即钉死）、② 首屏按路由拆包 + 空闲预取（04 §11.16）。

**新建（1 个）**：`app/routes.tsx` 85 行（预算 ~60；含代码审查后补的「等 SW 接管再预取」一段）—— 七个页面的 `lazy()` 与空闲预取共用的 `PAGE_LOADERS` + `prefetchRoutes()`。

**改动（3 个）**：

| 文件 | 现 → 实测 | 改什么 |
|---|---|---|
| `app/App.tsx` | 51 → 62 | 七个静态 import → 从 `routes.tsx` 取；`useEffect` 里调 `prefetchRoutes()` |
| `app/AppShell.tsx` | 129 → 134 | `<Outlet />` 外套 `Suspense`（骨架屏兜底，导航不闪） |
| `package.json` / `package-lock.json` | — | `xlsx` 0.18.5（npm 停更版）→ 0.20.3（CDN tarball URL）；**顺带移除 8 个传递依赖**（word/wmf/ssf/frac/crc-32/codepage/cfb/adler-32），`npm audit` 从 1 个 high 归零 |

**依赖方向**：`App → routes.tsx →（动态）features/*`、`AppShell → Skeleton`。无新环。

**测试账**：不变（469）—— 本批次是路由与依赖层改动，不新增纯逻辑。

### 5.8 B9 受影响文件与尺寸标注（2026-09-20 定稿）

沿用 §5.1 纪律。本批次：AI 配置加服务商预设（04 §11.17）+ 把设置页里超线的两块拆成独立文件。

**新建（4 个）**：

| 文件 | 预计行数 | 内容 |
|---|---|---|
| `features/settings/ai-presets.ts` | ~70 | 预设表 + `matchPresetId()` 反查（纯逻辑，可 `node --test`） |
| `features/settings/ai-presets.test.ts` | ~60 | 上者用例 |
| `features/settings/AiSection.tsx` | ~170 | 从 SettingsPage 迁出的 AI 配置区 + 预设按钮行（`BorrowerSection` / `SnapshotSection` 同款做法） |
| `features/settings/ClearDataSection.tsx` | ~90 | 从 SettingsPage 迁出的「清空数据」区（自带二次确认；`counts` 由调用方传入，不重查） |

**改动（1 个）**：`features/settings/SettingsPage.tsx` **599 → 450 行**（**减量**：AI 区近 85 行 + 清空数据区近 91 行迁出）—— 该文件长期越过 500 行硬上限（§5.1/§5.4 记为欠账），本次**真正回到线内**。

**依赖方向**：`SettingsPage → AiSection → ai-presets`、`SettingsPage → ClearDataSection`、`AiSection → platform/ai`。无新环。

**测试账**：469 → 预计 ~480（预设用例约 8）。

**实现后实测（2026-09-20 交付）**：全部 ≤300 行。

| 文件 | 预计 | 实测 |
|---|---|---|
| `features/settings/ai-presets.ts` | ~70 | 79 |
| `features/settings/ai-presets.test.ts` | ~60 | 65 |
| `features/settings/AiSection.tsx` | ~170 | 156 |
| `features/settings/ClearDataSection.tsx` | ~90 | 91 |
| `features/settings/SettingsPage.tsx` | 599 → 约 515 | **450**（首次拆完 AI 区为 512，仍在线上；再拆清空数据区后回到线内） |
...
- **截图验收**（`scratch/shots.mjs`，无头 Edge + CDP）：**必须等「地址已切换 + `h1` 是该页标题」两个条件都成立再拍**。`Page.navigate` 是异步的，新页面提交之前 `document` 还是**旧页面**，只等 `h1` 会抢拍到上一页——2026-09-20 踩过：后 4 张（深色/手机版）全是详情页，等于那几种视图压根没验到。脚本已改成双条件等待并会在等不到时打印警告。
...
| D11 | 元数据补全只走**用户自配的 AI 接口**，不接 Open Library / Google Books | ① 按原设计接两个免费源 ② 只接免费源不接 AI | 2026-09-19 实测：`openlibrary.org` 与 `www.googleapis.com` 在本机网络下均 `fetch failed`，而 `registry.npmjs.org`、`api.github.com` 可达——是网络限制而非偶发。两个源都连不上，接进来就是永远跑不通的死代码，还要白养解析与降级逻辑；用户可把 AI 接口配到国内服务商（DeepSeek / 通义等），实际可用。放弃的代价：没有配置 AI 的用户只能手填，这正是 04 §11.12 保留「填表但不建书 + 提示去配置」退路的原因 |
...
| D | 扫码录 ISBN（html5-qrcode，**离线可用**）+ AI 元数据兜底 | 扫一本真实书，ISBN 自动填好；已配置 AI 时书名/作者/出版自动填上，未配置则可手动补（免费源否决见 D11） | ✅ 2026-09-19 实现交付（规范：04 §11.12、05 §3.3 第 2 入口、D11；行数实测见 §5.5）；npm test 444/444、typecheck、build 全绿；html5-qrcode 独立动态分块（369 kB）未进首屏包；真机摄像头验收待用户执行 |
...
| B2 | 批量导入（粘贴/CSV）+ AI 元数据补全（OpenAI 兼容接口自配）+ 界面精致化 | 粘贴一段书单一键建完（重复跳过并报告）；配好外部 AI 后能从书名/ISBN 一键补全元数据；界面质感升级且深浅色正常 | ✅ 已交付（2026-09-20 补记）：三块内容分别落在后续批次 —— 批量导入由 B3/B4 扩到 xlsx / Word / 多格式（05 §2），AI 补全按 05 §3 落地并由 D/D3 接了两个新入口，界面与视觉由 B5/B6 完成（04 §11.9—§11.11）。原「🔨 进行中」标记是没跟上交付的旧状态 |
...
| B5 | 图标与界面打磨：木刻版画应用图标（重写 make-icons.mjs）+ 封面展示升级 + 空态插画 + 骨架屏 + 微交互 | 手机桌面图标换成木刻版画风；封面有比例框与阴影；空态有插画；列表加载不闪（骨架屏）；卡片/按钮有微反馈 | ✅ 2026-09-13 实现交付（规范：04 §11.9/§11.10）；npm test 425/425、typecheck、build 全绿；真机观感待用户过目 |
| B6 | 美术方向：纸·墨·朱（界面与应用图标同一套颜色体系）+ 不变量 I11（库内 authors/tags 形状韧性） | 界面变成宣纸底/暖墨字/靛墨主色/朱砂点校；标题转衬线；圆角与阴影收敛（版画无言调）；坏数据不再让界面崩 | ✅ 2026-09-13 实现交付（规范：04 §11.11、02 §6 I11） |
| B7 | 新增页精简：默认层（动作行 + 书名/作者/出版社/位置）+ 「更多信息（可选）」折叠其余九个字段 | 打开新增页一屏内看到要填的字段；展开后九个字段与今天逐项一致；收起时有**本次新填/改过**的项则显示「已填 N 项」（品相/标签一律不计） | ✅ 2026-09-20 实现交付（规范：04 §11.13；行数实测见 §5.6）；真机观感待用户过目 |
| D2 | 拍照识别：相册 / 文件里的条码照片 → 本地解码 → 与实时扫码同一条链路 | 选一张含条码的照片能填出 ISBN 并触发查库 / AI 补全；无条码照片给人话提示；照片不出本机 | ✅ 2026-09-20 实现交付（规范：04 §11.12 照片识别）；真机验收待用户（相册照片解码） |
...
| B11 | 搜索页「筛选」折叠（位置/标签/状态收进折叠区） | 手机首屏能看到结果；收起时徽章说清「N 项生效」，不制造隐藏筛选陷阱 | ✅ 2026-09-20 实现交付（规范：04 §11.19）；npm test 477/477、typecheck、build 全绿；CDP 驱动交互验过两处徽章；真机待用户 |
