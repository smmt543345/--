...
| B8 | 旧账三件：B2 状态订正、xlsx 换 SheetJS CDN 版、首屏按路由拆包 + 空闲预取 | 文档与交付实情一致；`npm audit` 归零；入口 chunk 明显变小且断网仍能进各页 | ✅ 2026-09-20 实现交付（规范：04 §11.16、01 §5.7）；入口 553.11 → 414.88 kB（gzip 173.28 → 133.06 kB）、audit 归零、npm test 469/469；真机断网验收待用户 |
